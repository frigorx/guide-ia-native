...
**⚠ N'allez pas directement à l'action.** Lisez le chapitre d'abord — déployer un chatbot pour des élèves engage votre responsabilité pédagogique et le cadre RGPD : c'est le contexte du chapitre qui le sécurise. Le téléchargement du fichier `.md` et le prompt à coller sont rassemblés à la fin, dans la [section 12](#radio-guidage).
...
12. [**Radio-guidage — créer son chatbot pédagogique**](#radio-guidage)
13. [Vous avez un chatbot pédagogique. Et maintenant ?](#et-maintenant)
14. [Ressources et liens](#ressources)
...
Ce chapitre suppose acquis les Chapitres 1 et 2 : Ollama installé, Open WebUI accessible, au moins un modèle téléchargé, et la maîtrise des skills. Tout ce qui suit se construit **dans Open WebUI**, sans installation supplémentaire.
...
**Le levier essentiel** 90 % de la qualité d'un chatbot pédagogique se joue dans le **system prompt**. Un excellent system prompt sur un modèle moyen bat largement un mauvais system prompt sur un modèle excellent. C'est l'endroit où l'enseignant injecte sa pédagogie. Le [Chapitre 2](chapitre-2.md) a posé le concept de skill ; ici on l'attache à une posture.
...
Le system prompt fixe la *posture*. Le skill apporte le *contenu* : référentiel précis de la matière, barème de l'enseignant, exemples canoniques de productions attendues, lexique métier. C'est l'addition des deux qui rend le tuteur crédible. Si la notion de skill ne vous est pas familière, revenez au [Chapitre 2](chapitre-2.md) avant de poursuivre.
...
## 12. Radio-guidage — créer son chatbot pédagogique avec une IA cloud

Vous avez lu le chapitre, vous connaissez les enjeux et les garde-fous. Voici l'outil pour **construire votre chatbot** : un prompt à coller dans une IA cloud, qui devient votre assistant patient et vous accompagne étape par étape — du diagnostic du cadre établissement au premier test.

### 12.1. Marche à suivre
...
5. **Suivez les questions de l'IA**, une étape à la fois. La configuration porte sur des fichiers et des réglages — **aucune donnée d'élève n'est échangée avec l'IA cloud**.

### 12.2. Le prompt à copier
...
OBJECTIF FINAL
Créer un chatbot pédagogique destiné à mes élèves, dans Open WebUI.
À la fin de cette session, j'aurai :
- Un « modèle personnalisé » Open WebUI avec un nom, une icône,
  une description.
- Un system prompt soigné qui cadre la posture pédagogique
  (socratique, sans donner la réponse directement, vocabulaire
  adapté au niveau, interdiction d'inventer, sortie de rôle en
  cas d'évocation de mal-être).
- Un skill matière couplé (référentiel, exemples canoniques).
- Un premier test avec une question d'élève réaliste.

PREMIÈRE ÉTAPE OBLIGATOIRE — DIAGNOSTIC PRÉALABLE
a) Vérifie que j'ai Ollama + Open WebUI fonctionnels (issu du
   Chapitre 1). Si non, oriente-moi vers le Chapitre 1.
b) Vérifie que je sais écrire un skill (issu du Chapitre 2). Si
   non, oriente-moi vers le Chapitre 2.
c) Demande-moi :
   - Quelle matière, quel niveau, quel public ?
   - Quels aménagements spécifiques (DYS, TDAH, SEGPA, AESH
     absente) ?
   - Quel cadre établissement est validé (chef d'établissement,
     RUPN, DPO, charte, info familles) ?
d) Si le cadre établissement n'est pas validé, EXPLIQUE que
   l'usage doit rester strictement personnel sans connexion
   d'élèves, et oriente-moi vers le bandeau de précaution en
   tête du chapitre.

Ne PROPOSE PAS de déployer un chatbot multi-postes sans
validation du cadre établissement.
...
## 13. Vous avez un chatbot pédagogique. Et maintenant ?
